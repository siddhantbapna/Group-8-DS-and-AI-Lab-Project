...
if "%choice%"=="1" (
    ...
    echo.
    pika\Scripts\python.exe scripts\quick_train_all.py
) else if "%choice%"=="2" (
    echo.
    ...
    echo.
    pika\Scripts\python.exe scripts\train_all_models.py
) else if "%choice%"=="3" (
    echo.
    ...
    if "%cv%"=="y" (
        pika\Scripts\python.exe scripts\train_all_models.py --models %models% --epochs %epochs%
    ) else (
        pika\Scripts\python.exe scripts\train_all_models.py --models %models% --epochs %epochs% --no-cv
    )
) else (
